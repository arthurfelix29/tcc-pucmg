...
namespace MicrosservicoAdministrativo.IntegrationTests.Controllers.V2
{
	...
	public class PerfilUsuarioControllerIntegrationTest
	{
		...
		public async void PerfilUsuarioControllerIntegrationTest_ConsigoPostEGetByIdEDelete()
		{
			...
			{
				var postResponse = await client.PostAsJsonAsync("/api/v2/PerfilUsuario", perfilUsuario);
				var created = await postResponse.Content.ReadAsJsonAsync<PerfilUsuario>();

				var getResponse = await client.GetAsync("/api/v2/PerfilUsuario/" + created.Id);
				var fetched = await getResponse.Content.ReadAsJsonAsync<PerfilUsuario>();

				var deleteResponse = await client.DeleteAsync("/api/v2/PerfilUsuario/" + fetched.Id);
				var getAllResponse = await client.GetAsync("/api/v2/PerfilUsuario/");
		...
		public async void PerfilUsuarioControllerIntegrationTest_ConsigoPostEGetAllEDelete()
		{
			...
			using (var client = _server.CreateClient().AcceptJson())
			{
				...
				var deleteResponse = await client.DeleteAsync("/api/v2/PerfilUsuario/" + created.Id);
				var getAllForDeleteResponse = await client.GetAsync("/api/v2/PerfilUsuario/");
