...
namespace MicrosservicoAdministrativo.IntegrationTests.Controllers.V2
{
	...
	public class UsuarioControllerIntegrationTest
	{
		...
		[Fact]
		public async void UsuarioControllerIntegrationTest_ConsigoPostEGetById()
		{
			...
			{
				var postResponse = await client.PostAsJsonAsync("/api/v2/Usuario", usuario);
				var created = await postResponse.Content.ReadAsJsonAsync<Usuario>();

				var getResponse = await client.GetAsync("/api/v2/Usuario/" + created.Id);
		...
		[Fact]
		public async void UsuarioControllerIntegrationTest_ConsigoPostEGetAll()
		{
			var usuario = new Usuario
			{
				Id = Guid.NewGuid(),
			...
			using (var client = _server.CreateClient().AcceptJson())
			{
				...
				var getAllResponse = await client.GetAsync("/api/v2/Usuario/");
		...
		[Fact]
		public async void UsuarioControllerIntegrationTest_ConsigoPostEPutEGetById()
		{
			var usuario = new Usuario
			{
				Id = Guid.NewGuid(),
			...
			using (var client = _server.CreateClient().AcceptJson())
			{
				...
				var putResponse = await client.PutAsJsonAsync("/api/v2/Usuario/" + created.Id, usuario);

				var getResponse = await client.GetAsync("/api/v2/Usuario/" + usuario.Id);
				var fetched = await getResponse.Content.ReadAsJsonAsync<Usuario>();
		...
		[Fact]
		public async void UsuarioControllerIntegrationTest_ConsigoPost()
		{
			var usuario = new Usuario
			{
				Id = Guid.NewGuid(),
