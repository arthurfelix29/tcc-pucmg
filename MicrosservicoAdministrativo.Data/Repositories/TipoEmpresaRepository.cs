...
namespace MicrosservicoAdministrativo.Data.Repositories
{
	...
	public class TipoEmpresaRepository : IRepository<TipoEmpresa, string>
	{
		...
		private static readonly string CollectionId = "TipoEmpresaCollection";
		...
		public async Task<IEnumerable<TipoEmpresa>> GetItemsFromCollectionAsync()
		{
			var documents = docClient.CreateDocumentQuery<TipoEmpresa>(
				  UriFactory.CreateDocumentCollectionUri(DatabaseId, CollectionId),
				  new FeedOptions { MaxItemCount = -1 })
				  .AsDocumentQuery();

			List<TipoEmpresa> tiposEmpresa = new List<TipoEmpresa>();

			while (documents.HasMoreResults)
				tiposEmpresa.AddRange(await documents.ExecuteNextAsync<TipoEmpresa>());

			return tiposEmpresa;
		}

		public async Task<TipoEmpresa> GetItemFromCollectionAsync(string id)
		{
			try
			{
				Document doc = await docClient.ReadDocumentAsync(UriFactory.CreateDocumentUri(DatabaseId, CollectionId, id));

				return JsonConvert.DeserializeObject<TipoEmpresa>(doc.ToString());
			}
		...
		public async Task<TipoEmpresa> AddDocumentIntoCollectionAsync(TipoEmpresa item)
		{
			try
			{
				...
				return JsonConvert.DeserializeObject<TipoEmpresa>(res.ToString());
			}
			catch (Exception ex)
			{
				throw ex;
		...
		public async Task<TipoEmpresa> UpdateDocumentFromCollection(string id, TipoEmpresa item)
		{
			try
			{
				...
				return JsonConvert.DeserializeObject<TipoEmpresa>(data);
			}
			catch (Exception ex)
			{
				throw ex;
