...
namespace MicrosservicoAdministrativo.Data.Repositories
{
	...
	public class EmpresaRepository : IRepository<Empresa, string>
	{
		...
		private static readonly string CollectionId = "EmpresaCollection";
		...
		public async Task<IEnumerable<Empresa>> GetItemsFromCollectionAsync()
		{
			var documents = docClient.CreateDocumentQuery<Empresa>(
			...
			while (documents.HasMoreResults)
				empresas.AddRange(await documents.ExecuteNextAsync<Empresa>());

			return empresas;
		}

		public async Task<Empresa> GetItemFromCollectionAsync(string id)
		{
			try
			{
				Document doc = await docClient.ReadDocumentAsync(UriFactory.CreateDocumentUri(DatabaseId, CollectionId, id));

				return JsonConvert.DeserializeObject<Empresa>(doc.ToString());
			}
		...
		public async Task<Empresa> AddDocumentIntoCollectionAsync(Empresa item)
		{
			try
			{
				...
				return JsonConvert.DeserializeObject<Empresa>(res.ToString());
			}
			catch (Exception ex)
			{
				throw ex;
		...
		public async Task<Empresa> UpdateDocumentFromCollection(string id, Empresa item)
		{
			try
			{
				...
				return JsonConvert.DeserializeObject<Empresa>(data);
			}
			catch (Exception ex)
			{
				throw ex;
